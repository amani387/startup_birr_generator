...
const REFERRAL_REWARDS_DISPLAY = VIP_REFERRAL_REWARDS.filter((row) => row.level <= 5);
        ...
        <table className="w-full min-w-[420px] text-sm">
          <thead className="sticky top-0 z-10 bg-surface">
            <tr className="border-b border-border text-left text-muted">
              <th className="pb-3 font-medium">VIP Package</th>
              <th className="pb-3 font-medium">Price</th>
              <th className="pb-3 font-medium">Invitation Reward</th>
            </tr>
          </thead>
          <tbody>
            {REFERRAL_REWARDS_DISPLAY.map((row) => (
              <tr key={row.level} className="border-b border-border/50">
                <td className="py-3 font-medium">{row.name} invitation</td>
